...
public class Dijkstra {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			V[] vertices = new V[nV];
			E[] edges = new E[2*nE]; //2*nE alleen bij undirected graph
			...
			for (int j = 0; j < nE; j++) {
				...
				edges[2*j+1] = new E(vertices[start], w); //for undirected graph
				vertices[start].adj.add(edges[2*j]);
				vertices[end].adj.add(edges[2*j+1]); //for undirected graph
			}
